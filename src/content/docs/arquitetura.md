...
`src/mcp.zig` adds a third entry point: `agent-tts mcp` runs a stdio JSON-RPC 2.0 loop. Newline-delimited JSON (NOT LSP-style `Content-Length` framing — that is the MCP convention for stdio transport). Three methods land in v1.5:
...
```
src/
  main.zig         # entry, argv routing, ttfa-bench + piper-test + mcp subcommands
  ...
  mcp.zig          # MCP server (stdio JSON-RPC 2.0, v1.5+)
```
