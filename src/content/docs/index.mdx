...
<CardGrid>
  <Card title="MCP server bundled" icon="puzzle">
    `agent-tts mcp` — stdio JSON-RPC. 10 tools: `say`, `queue`, `skip`, `clear`, `voices`, `say_stream`, `pause`, `resume`, `replay`, `history`. Drop-in for Claude Code / Cursor / Cline. [Read →](/mcp/)
  </Card>
  ...
  <Card title="Menubar floating player" icon="laptop">
    macOS NSStatusItem + floating NSPanel. Skip / Replay / voice picker — without opening a terminal. [Read →](/menubar/)
  </Card>
  <Card title="SSML & prosody" icon="document">
    W3C SSML 1.1 subset: `<emphasis>`, `<break>`, `<prosody>`, `<say-as>`. Parse < 0.2 µs / 280 chars. Works on `say` and on Piper (length-scale + silence). [Read →](/motor/)
  </Card>
</CardGrid>
...
Living project. Eighteen base + six patch milestones already shipped — see the [Roadmap](/roadmap/) and [Changelog](/changelog/) for the full timeline. The current state of every feature lives in [Architecture](/arquitetura/), [TTS engine](/motor/), [MCP server](/mcp/), [Menubar UI](/menubar/), and [Playground](/playground/).
