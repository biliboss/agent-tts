---
title: What's next
description: Four next versions of agent-tts — multilingual, streaming, cross-platform, and voice cloning. (v1.5 MCP server shipped — see Changelog.)
---

## TL;DR

v1.0 shipped the runtime: single binary, neural Pt-BR by default, persistent queue, launchd auto-start, sub-100 ms warm synth. v1.5 added the [MCP server](/mcp/) — Claude Code, Cursor, Cline talk to agent-tts directly. **What follows is about reach, not plumbing.** Four themes pulled out of the v1.0 backlog and pointed at the audiences that will care.
